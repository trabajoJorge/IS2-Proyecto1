...
public class DataAccess  {
	...
	public void initializeDB(){
		...
		try {
		   ...
		   if (month==12) { month=0; year+=1;}  
			...
			Cliente cli = new Cliente("u", "", "user@mail.com", (float) 10.11 , "Usuario1 Apellido1", "ES12 3445 4567 2320 3456 8654 1234", "1245 6543 3456 8766");
	...
	public boolean insertAnswer(Answer pan) {
		System.out.println(">> DataAccess: insertAnswer=> question= "+pan.getQuestion().getQuestion()+" answer="+pan.getAnswerText());
		...
		try {
			...
			//db.persist(q);
			db.persist(ans); // db.persist(q) not required when CascadeType.PERSIST is added in questions property of Event class
							// @OneToMany(fetch=FetchType.EAGER, cascade=CascadeType.PERSIST)
			db.getTransaction().commit();
			return true;
		} catch (Exception e) {
	...
	public int createApuesta(Answer pselectedAnswer, Cliente pselectedClient, Float pselectedAmount, Question pselectedQuestion) {
			...
			if (pselectedAmount < 0){
				...
				if (pselectedAmount < pselectedQuestion.getBetMinimum()) {
					...
					if (pselectedAmount >= clientdb.getSaldo()) {
						...
						System.out.println(">> DataAccess: createApuesta=> answer= "+pselectedAnswer+" client= "+clientdb.getUsername()+" amount="+pselectedAmount+"â‚¬");
	...
	public boolean definirResultados(Event pselectedEvent, Question pselectedQuestion, Answer pselectedAnswer, Admin pcurrentAdmin) {
		...
		try {
					...
					for(Apuesta apu : ArrayListApuestas) {
						...
						if (apu.getEstado()!=2) {
							...
							if (apu.getAnswer().getAnswerNumber()==pselectedAnswer.getAnswerNumber()) {
								...
								System.out.println("\nAcredita al cliente "+cliente.getUsername()+" un total de "+apu.getAmount()*pselectedAnswer.getCoeficienteGanancia()+"â‚¬ ("+apu.getAmount()+"â‚¬ x "+pselectedAnswer.getCoeficienteGanancia()+")");
			...
			JOptionPane.showMessageDialog(null, "DataAccess >> Asignar Resultados y Pagos >> Catch: "+e.getMessage()); //FIX-ME! Comentar la lÃ­nea
			return false;
		}
		...
		try {
			...
			//JOptionPane.showMessageDialog(null, "ArrayListQuestions: "+ArrayListQuestions.isEmpty()); //FIX-ME! Comentar la lÃ­nea
	...
	public boolean acreditarSaldo(Cliente pClient, float pMonto, int pPin, String pTipo) {
		...
		if (pTipo=="t") {
			...
			//SIMULA UNA TRANSACCIÃ“N CON TARJETA DE CRÃ‰DITO Y ENVÃ�A NÃšMERO DE TARJETA Y PIN
			db.getTransaction().begin();
			Cliente cli = db.find(Cliente.class, pClient);
			cli.setSaldo(cli.getSaldo()+pMonto);
			db.getTransaction().commit();
